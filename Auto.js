...
function parseTimeInput(timeStr) {
    ...
    // Regular expression to match time formats: 12:30 PM, 1:45 am, 23:30, etc.
    ...
    if (period) {
        ...
        if (period === 'AM') {
            ...
            if (hour !== 12) hour += 12; // 12:00 PM stays as 12:00
        }
    } else {
        // 24-hour format
        if (hour < 0 || hour > 23) {
...
// --- Get user input for scheduling time ---
async function getUserScheduleTime() {
    ...
    return new Promise((resolve) => {
        ...
        console.log('Enter the time for daily batch run.');
        console.log('Format examples: 11:30 PM, 11:30 pm, 23:30, 9:00 AM, 09:00 am\n');
        
        const askTime = () => {
            rl.question('Enter time (or type "now" to run immediately): ', (answer) => {
                ...
                const parsed = parseTimeInput(answer);
                if (parsed) {
                    console.log(`Scheduled time set to: ${parsed.hour.toString().padStart(2, '0')}:${parsed.minute.toString().padStart(2, '0')} (24-hour format)`);
                    rl.close();
                    resolve(parsed);
                } else {
                    console.log('Invalid time format. Please try again.');
                    console.log('Examples: 11:30 PM, 11:30 pm, 23:30, 9:00 AM, 09:00 am');
                    askTime(); // Ask again
                }
            });
        };
...
(async () => {
    ...
    // Get scheduling time from user
    const scheduleTime = await getUserScheduleTime();
    
    if (scheduleTime.runNow) {
        console.log('\nStarting batch run immediately...');
        try {
            await runBatch();
        } catch (err) {
        ...
        // Schedule daily run at user-specified time
